...
#################################################################################
 ...
 # description      : generate ConvertAB_RCA verilog file
 #                    
 ...
 # function         : @instance_rca_0_ports         : define the SecRCA ports
 # function         : @instance_rca_1_ports         : define the SecRCA ports
...
class ConvertAB_RCA:
    ...
    def instance_convertabx_0_ports(self):
        ports = [Port('clk_i'   , 'clk_i'),
                 ...
                 Port('i_dvld'  , 'i_dvld'),
                 Port('i_rvld'  , 'i_rvld'),
                 Port('i_n'     , "i_n[{}+:{}]".format(self.list_par.ll_r_start*1,self.list_par.ll_r_len*1)),
    ...
    def instance_convertaby_ports(self):
        ports = [Port('clk_i'   , 'clk_i'),
                 ...
                 Port('i_dvld'  , 'i_dvld'),
                 Port('i_rvld'  , 'i_rvld'),
                 Port('i_n'     , "i_n[{}+:{}]".format(self.list_par.rl_r_start*1,self.list_par.rl_r_len*1)),
    ...
    def instance_rca_0_ports(self):
        ports = [Port('clk_i'   , 'clk_i'),
                 Port('rst_ni'  , 'rst_ni'),
                 Port('i_dvld'  , 'vy'),
                 Port('i_rvld'  , 'i_rvld'),
                 Port('i_n'     , "i_n[{}+:{}]".format(self.list_par.rca_r_start*1, self.list_par.rca_r_len*1)),
    ...
    def write(self):
        ...
        if self.list_par.rl_convert_name == "":
            ...
            raw += add_comment("Connect input port to right leaf data")
        ...
        if self.list_par.rl_latency >= self.list_par.ll_latency:
            ...
            self.inst_cnt       = self.inst_cnt + int(1)
            inst_parameters     = [ Parameter('W',"{}".format(width) ), Parameter('N','{}'.format(delay))]
            inst_ports          = instance_shr1(logics[6].get_name(h), ports[1].get_name(), logics[3].get_name(), logics[7].get_name(), logics[4].get_name(), width, 0 )
            inst_fraw           = ""
            inst_praw           = ""   
            self.verilog_writer.add(Instance(inst_module_name,
                                             inst_name,
        ...
        # rca inst
        raw   = ""
        raw  += add_comment("Do a RCA instance")
        self.verilog_writer.add(Raw(raw)) 

        if self.list_par.exist_odvld == int(1):
            inst_module_name    = "{}_1".format(self.list_par.rca_name)
            inst_name           = "u{}_{}".format(self.inst_cnt, inst_module_name)
            self.inst_cnt       = self.inst_cnt + int(1)
            inst_parameters     = []                        
            inst_ports          = self.instance_rca_1_ports()
            inst_fraw           = ""
            inst_praw           = ""    
            self.verilog_writer.add(Instance(inst_module_name,
                                             inst_name,
            ...
            inst_module_name    = "{}_0".format(self.list_par.rca_name)
            inst_name           = "u{}_{}".format(self.inst_cnt, inst_module_name)
            self.inst_cnt       = self.inst_cnt + int(1)
            inst_parameters     = []                        
            inst_ports          = self.instance_rca_0_ports()
            inst_fraw           = ""
            inst_praw           = ""    
            self.verilog_writer.add(Instance(inst_module_name,
                                             inst_name,
