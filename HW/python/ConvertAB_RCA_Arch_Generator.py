...
from ConvertAB_RCA_Generator import *
from Expand1_Generator import *
from Expand2_Generator import *
from SecAnd_PINI1_Generator import SecAnd_PINI1
from SecRCA_1b_Generator import SecRCA_1b
from SecRCA_Generator import SecRCA



#################################################################################
 # class name       : ConvertAB_RCA
 # description      : Initialize the parameters of ConvertAB_RCA architecutre
 #                    write files
 #
 ...
 # 
#################################################################################
class ConvertAB_RCA_arch:
    def __init__(self, shares, width, iter_depth, exist_odvld=int(1)):
        self.shares           = shares
        self.width            = width
        self.id               = iter_depth   
        self.exist_odvld      = exist_odvld
        self.name             = "ConvertAB_RCA_n{}k{}".format(self.shares, self.width)
        self.rca_w            = self.width-1
        ...
        if self.shares <= int(1):
            self.rca_name           = ""
            self.rca_latency        = int(0)
            self.rca_r_start        = int(0)
            self.rca_r_len          = int(0)
        else:
            self.rca_name           = "SecRCA_n{}k{}".format(self.shares, self.width)
            self.rca_latency        = self.rca_w*2
            self.rca_r_start        = self.rl_r_start + self.rl_r_len
            self.rca_r_len          = self.rca_w*self.n_r        

        self.max_leaf_latency = max(self.ll_latency, self.rl_latency)
        self.latency          = self.max_leaf_latency + self.rca_latency
        self.r_start          = int(0)
        self.r_len            = self.rca_r_start + self.rca_r_len
    ...
    def add_left_leaf(self):
        ...
        if self.ll_shares > int(1):
            self.ll_convert_name    = "ConvertAB_RCA_n{}k{}".format(self.ll_shares, self.ll_width)
            handler                 = ConvertAB_RCA_arch(self.ll_shares, self.width, self.id+int(1))
    ...
    def add_right_leaf(self):
        ...
        if self.rl_shares > int(1):
            self.rl_convert_name    = "ConvertAB_RCA_n{}k{}".format(self.rl_shares, self.rl_width)
            handler                 = ConvertAB_RCA_arch(self.rl_shares, self.width, self.id+1)
    ...
    def showtree(self):
        ...
        print("|","-"*(self.id+1),">","rca_r_start      =",self.rca_r_start,"\n") 
        print("|","-"*(self.id+1),">","rca_r_len        =",self.rca_r_len,"\n") 
    ...
    def write(self, fp_filelist):
        #################################################################################
        g_inst = ConvertAB_RCA(self.name, self.shares, self.width, self)
        g_inst.write()
        ...
        #################################################################################        
        g_inst = SecRCA(self.rca_name, self.shares, self.width, self.exist_odvld)   
