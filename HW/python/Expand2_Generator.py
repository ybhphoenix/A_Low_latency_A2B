...
#################################################################################
 # class name       : Expand2
 # description      : generate Expand2 verilog file
 #                    
...
class Expand2:
    ...
    def write(self):
        ...
        #################################################################################
        raw  = ""
        raw += add_comment("for(i=0;i<{};i++)\n".format(self.n-self.n2)+"{\n  xp[i]=0;\n}")
        raw += assigns_zeros(ports[1].get_name(), 0, self.n-self.n2, self.width) 
        raw += add_comment("for(i=0;i<{};i++)\n".format(self.n2)+"{"+"\n  xp[{}-{}+i]=x[i];\n".format(self.n, self.n2)+"}")       
        raw += assigns_ofs(ports[1].get_name(),ports[0].get_name() , self.n-self.n2, self.n, 0, self.width)
